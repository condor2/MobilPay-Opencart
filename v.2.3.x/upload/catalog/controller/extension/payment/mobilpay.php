	...
	public function callback() {
		...
		if (strcasecmp($_SERVER['REQUEST_METHOD'], 'post') == 0)
		{
			if(isset($_POST['env_key']) && isset($_POST['data']))
			{
				...
				if ($this->config->get('mobilpay_test')) {
					$privateKeyFilePath = DIR_APPLICATION .'Mobilpay/cert/sandbox.'.$this->config->get('mobilpay_signature').'private.key';
				} else {
					$privateKeyFilePath = DIR_APPLICATION .'Mobilpay/cert/live.'.$this->config->get('mobilpay_signature').'private.key';
				}
				...
				try {
					...
					if ($objPmReq->objPmNotify->errorCode == 0) { 
						...
						case 'confirmed':
							...
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mobilpay_order_status_id'));
...
//							$this->model_checkout_order->update($order_id, $this->config->get('mobilpay_order_status_id'), '', TRUE);
						...
						case 'paid_pending':
							...
							$errorMessage = $objPmReq->objPmNotify->getCrc();
							$this->model_checkout_order->confirm($order_id, $this->config->get('mobilpay_order_status_paid_pending_id'));
						...
						case 'paid':
							...
							$errorMessage = $objPmReq->objPmNotify->getCrc();
							$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mobilpay_order_status_paid_id'));
